...
export default function App() {
  ...
  async function fetchTasks() {
    try {
      setTasks(await getAllTasks(user.uid));
    } catch (e) {
      setError(e.message);
    }
  }
  ...
  async function handleSubtaskToggle(taskId, subtasks) {
    const task       = tasks.find(t => t.id === taskId)
    const allDone    = subtasks.length > 0 && subtasks.every(s => s.done)
    const pastDue    = task?.dueDate && new Date(task.dueDate + 'T23:59:59') < new Date()
    const autoFinish = allDone && pastDue && task?.status !== 'DONE'

    setTasks(prev => prev.map(t =>
      t.id === taskId ? { ...t, subtasks, ...(autoFinish ? { status: 'DONE' } : {}) } : t
    ))
    try {
      await updateSubtasks(user.uid, taskId, subtasks)
      if (autoFinish) await patchStatus(user.uid, taskId, 'DONE')
    } catch (e) {
      setError(e.message)
      fetchTasks()
    }
  ...
  async function handleStatusChange(id, status) {
    try {
      await patchStatus(user.uid, id, status);
      fetchTasks();
    } catch (e) {
      setError(e.message);
    }
  ...
  async function handlePriorityChange(id, priority) {
    try {
      await updateTask(user.uid, id, { priority });
      fetchTasks();
    } catch (e) {
      setError(e.message);
    }
  ...
  async function handleCategoryChange(id, categoryId) {
    try {
      await updateTask(user.uid, id, { categoryId });
      fetchTasks();
    } catch (e) {
      setError(e.message);
    }
  ...
  async function handleRenameCategory(id, name) {
    try {
      await renameCategory(user.uid, id, name);
      fetchCategories();
    } catch (e) {
      setError(e.message);
    }
  ...
  async function handleDeleteCategory(id) {
    if (
      !window.confirm(
        "Delete this category and all its tasks? This cannot be undone.",
      )
    )
      return;
    try {
